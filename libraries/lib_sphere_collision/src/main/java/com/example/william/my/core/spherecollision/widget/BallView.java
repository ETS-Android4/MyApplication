...
import com.example.william.my.core.spherecollision.R;
...
public class BallView {
    ...
    private final ViewGroup mViewGroup;//父控件
    ...
    public BallView(Context mContext, ViewGroup viewGroup) {
    ...
    private void initCircleWorldBounds(Canvas canvas) {
        // 绘制圆形边框
        canvas.drawCircle(Math.min(pWidth, pHeight) / 2, Math.min(pWidth, pHeight) / 2, Math.min(pWidth, pHeight) / 2, paint);
        if (isInitWorld) {
            return;
        }
        BodyDef bodyDef = new BodyDef();
        ...
        //设置圆形刚体边界
        ArrayList<float[]> positions = polygon2(36, Math.min(pWidth, pHeight) / 2);
        for (int i = 0; i < positions.size(); i++) {
            float[] xy = positions.get(i);
            float x = xy[0];
            float y = xy[1];
     ...
     * @return
     */
    public ArrayList<float[]> polygon(int n, float startX, float startY, int r) {
     ...
     * @return
     */
    public ArrayList<float[]> polygon2(int n, int r) {
